...
#let project(
  ...
  show table: it => {
    let other_table = it.stroke == none
    if it.children.len() > 0 and it.children.at(it.children.len() - 1).func() == table.hline or other_table {
      it
    } else {
      let items = it.fields()
      let children = items.remove("children")
      table(
        ..items,
        ..children,
        table.hline(stroke: (
  ...
  show heading.where(level: 3): set block(above: 2.0em, below: 0.2em)
 ...
 if ai_clause {
     import "ai_usage_clause.typ": confidentialityClauseWith
    confidentialityClauseWith(lang: lang)
  }
